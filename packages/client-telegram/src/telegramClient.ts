...
export class TelegramClient {
    ...
    public async start(): Promise<void> {
        console.log("TestPP::Telegram client initialized");
        //elizaLogger.log("TestPP::Telegram client initialized");
    ...
    private setupMessageHandlers(): void {
        ...
        this.bot.on("document", (ctx) => {
            ...
            elizaLogger.info("📎 Received document message:");
            elizaLogger.info(`  File Name: ${document.file_name}`);
            elizaLogger.info(`  File ID: ${document.file_id}`);
            elizaLogger.info(`  File Size: ${document.file_size} bytes`);
            elizaLogger.info(`  MIME Type: ${document.mime_type}`);
        
            // Optionally, log the chat ID and user ID
            elizaLogger.info(`  Chat ID: ${ctx.chat.id}`);
            if (ctx.from) {
                elizaLogger.info(`  User ID: ${ctx.from.id}`);
                elizaLogger.info(`  Username: ${ctx.from.username}`);
            }
        });
